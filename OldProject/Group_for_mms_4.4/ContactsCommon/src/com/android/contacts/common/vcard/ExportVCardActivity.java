...
public class ExportVCardActivity extends Activity implements ServiceConnection,
    ...
    protected void onCreate(Bundle bundle) {
        super.onCreate(bundle);
         final boolean export2SD = getIntent().getExtras().getBoolean(VCardCommonArguments.EXPORT_TO_REMOVABLE_SD);
        // Check directory is available.
        // start Fix SEFOFAE-837. Update proper path
     if(export2SD){
     ...
     }// end  Fix SEFOFAE-837. Update proper path
        ...
        intent.putExtra(VCardCommonArguments.EXPORT_TO_REMOVABLE_SD, export2SD);
    ...
    protected void onPrepareDialog(int id, Dialog dialog, Bundle args) {
        if (id == R.id.dialog_fail_to_export_with_reason) {
            ...
            String msg = getProperMessage(mTargetFileName);
    ...
    /**
     * Fix SEFOFAE-270. Build dialog message with file name, removing prefix /storage/emulated/0</br>
     * See {@link VCardService}
     * @param fileName of the absolute path
     * @return message with tweaked prefix path
     */
    private String getProperMessage(String fileName) {
        StringBuffer buffer = new StringBuffer();
        if (fileName.startsWith("/storage/sdcard1")) {
            buffer.append(getString(R.string.removable_storage));
        } else {
            buffer.append(getString(R.string.internal_storage));
        }
        buffer.append(fileName.substring(fileName.lastIndexOf("/")));
        return getString(R.string.confirm_export_message, buffer.toString());
    }
    ...
    public void onConfigurationChanged(Configuration newConfig) {
        ...
        // Fix SEFOFAE-298. Survive over roation change
    }
}
