...
//aoran add end
	...
	//aoran delete result
        ...
        if ((isScreenOn && !mHeadless) || (isInjected && !isWakeKey)) {
            ...
            result = 0;
            if (down && isWakeKey && isWakeKeyWhenScreenOff(keyCode) && !shouldDisableScreenLight(keyCode)) { // haoran.wang,SFAE-562
                result |= ACTION_WAKE_UP;
                /// M:Screen on debugging mechanism @{
        ...
        // haoran.wang,SFAE-562,begin
        if (/*FeatureOption.TINNO_SMART_AWAKE_SUPPORT && */keyCode == KeyEvent.KEYCODE_KEYTP && down) {

            Log.d("haoran", "KEYCODE_KEYTP work begin");
            
            BufferedReader reader = null;

            try {
                reader = new BufferedReader(new FileReader("/proc/tgeseture_config"), 128);
                String status = reader.readLine();
                reader.close();

                if (Settings.Global.getInt(mContext.getContentResolver(),
                        Settings.Global.ENABLE_SMART_AWAKE_ENABLED, 0) == 1) {

                    Log.d("haoran", "Settings.Global.ENABLE_SMART_AWAKE_ENABLED=1");

                    if (shouldUnlock(status)) {
                        Intent intent_unlock = new Intent("android.intent.action.HALL_CHANGED");
                        intent_unlock.putExtra("state", 1);
                        mContext.sendBroadcast(intent_unlock);
                    }                        

                    Intent intent = new Intent("android.intent.action.SMART_AWAKE");
                    intent.putExtra("state", status);
                    mContext.sendBroadcast(intent);
                } else {
                    Log.d("haoran", "Settings.Global.ENABLE_SMART_AWAKE_ENABLED=0");
                }
            } catch (IOException e) {
                e.printStackTrace();
            }

            Log.d("haoran", "KEYCODE_KEYTP work finished");
        }
        // haoran.wang,SFAE-562,end		
        return result;
    }


    // haoran.wang,SFAE-562,begin
    boolean shouldDisableScreenLight(int keyCode) {
        if (keyCode == KeyEvent.KEYCODE_KEYTP) {
            return (false/*!FeatureOption.TINNO_SMART_AWAKE_SUPPORT*/
                || (Settings.Global.getInt(mContext.getContentResolver(), Settings.Global.ENABLE_SMART_AWAKE_ENABLED, 0) == 0));
        } else {
            return false;
        }
    }

    boolean shouldUnlock(String status) {
        if (status.indexOf("0xc") != -1
            || status.indexOf("0xm") != -1
            || status.indexOf("0xs") != -1) {
            return true;
        } else {
            return false;
        }
