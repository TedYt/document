...
public class ProximitySensor implements AccelerometerListener.OrientationListener,
    ...
    private boolean mIsPhoneIdle = false;
	...
	/// M: aoran add,for Tinno Gesture,Start @{
	//To enable proximity sensor when incoming state if promixy inconimg enabled
	if((android.provider.Settings.Secure
							.getInt(mContext.getContentResolver(), Settings.Global.ENABLE_GESTURE_SETTINGS_ENABLED,
									android.provider.Settings.Secure.PROMIXY_INCOMING_CALL_ENABLED, 0) == 1))
	{								
	   isOffhook = (InCallState.INCALL == state || InCallState.INCOMING == state
                || InCallState.OUTGOING == state);
	}
	 /// @} end modify	
		mIsPhoneIdle = InCallState.NO_CALLS == state;
    ...
    private void updateProximitySensorMode() {
        if (proximitySensorModeEnabled()) {
            synchronized (mProximityWakeLock) {
                ...
                android.util.Log.d("wmj2", "update=-----mIsPhoneIdle:"+mIsPhoneIdle);
                android.util.Log.d("wmj2", "update=-----screenOnImmediately:"+screenOnImmediately);
                android.util.Log.d("wmj2", "update=-----mIsPhoneOffhook:"+mIsPhoneOffhook);
//                if (mIsPhoneOffhook && !screenOnImmediately) {
				if (!mIsPhoneIdle && !screenOnImmediately) {
					android.util.Log.d("wmj2", "update=11111111");
					
					if ((mContext!=null && (Settings.Secure.getInt(mContext.getContentResolver(),
							Settings.Global.ENABLE_GESTURE_SETTINGS_ENABLED,
							Settings.Secure.PROMIXY_INCOMING_CALL_ENABLED, 0) == 1))
							|| (mIsPhoneOffhook)) {
						android.util.Log.d("wmj2", "update=2222222222222");
	                    ...
	                    */
	                    ...
	                    /// @}
							...
							Log.d(this,
									"updateProximitySensorMode: PROMIXY_INCOMING_CALL_ENABLED_DISABLE.");
					}
                } else {
